# Third-Party Trademark Notices

This repository references third-party database products and trademarks. These references are made under nominative fair use for identification purposes only. Percona is not affiliated with, sponsored by, or endorsed by any of the trademark owners listed below unless explicitly stated.

| Trademark | Owner |
| --- | --- |
| MongoDB® | MongoDB, Inc. |
| MySQL® | Oracle Corporation |
| PostgreSQL® | The PostgreSQL Community Association of Canada |
| Redis® | Redis Ltd. |
| MariaDB® | MariaDB plc / MariaDB Foundation |
| Valkey | The Linux Foundation |
| Kubernetes® | The Linux Foundation |
| CloudNativePG | CloudNativePG contributors / CNCF |
| Oracle® | Oracle Corporation |

All other trademarks are the property of their respective owners.

Percona®, Percona Server, Percona XtraDB Cluster, Percona XtraBackup, Percona Monitoring and Management (PMM), Percona Toolkit, Percona Backup for MongoDB, Percona ClusterSync, Percona Search, and Percona Operators are trademarks or registered trademarks of Percona LLC.
