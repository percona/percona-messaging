...
- Multi-environment operations: Running MySQL on Kubernetes or across multi-cloud requires specialized expertise. Percona Kubernetes Operator for MySQL has separate lines for Percona XtraDB Cluster and Percona Server for MySQL. The Percona Server for MySQL operator line remains in technical preview, while the Percona XtraDB Cluster line is production-focused. Together, they automate backups, scaling, and upgrades based on deployment requirements.
- Current shipped operator signals: Percona Operator for MySQL 1.1.0 (Percona Server for MySQL line) shipped with technical preview PITR and incremental backup capabilities plus compression updates, while Percona Operator for MySQL 1.19.1 (Percona XtraDB Cluster line) shipped with fixes such as the ProxySQL crash-loop scenario in cross-site replication setups. Detailed release-specific operator narrative is maintained in the Operators lane.
