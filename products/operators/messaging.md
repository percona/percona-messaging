...
- **Near-zero downtime major PostgreSQL upgrades:** Major version upgrades on large clusters traditionally mean multi-hour maintenance windows. Percona Operator for PostgreSQL supports major-version upgrades with a blue/green approach via logical replication, so applications stay available through a brief switchover and teams keep a rollback path instead of treating the upgrade as a planned outage. [PostgreSQL major version upgrade workflow](https://docs.percona.com/percona-operator-for-postgresql/latest/update-db-major.html)
