...
- MySQL operator path and current release context: Percona maintains separate operator lines for Percona XtraDB Cluster (production HA focus, including fixes such as the ProxySQL cross-site crash-loop scenario in 1.19.1) and Percona Server for MySQL (technical preview path, including PITR and incremental backup capabilities in 1.1.0).
