...
- **MongoDB on your terms, from Atlas to Kubernetes:** Teams leaving MongoDB Atlas, where infrastructure is managed and you have no control, can run Percona Server for MongoDB on Kubernetes with on-premises power and a SaaS-like experience. The Percona Operator for MongoDB automates MongoDB orchestration on Kubernetes, runs Percona ClusterSync for MongoDB to copy and keep data in sync while the app stays on the source, and handles ongoing Day 2 maintenance. You move the app when you are ready, with a much smaller cutover window than dump and restore.
...
- **Native transparent data encryption for PostgreSQL:** Percona Operator for PostgreSQL enables pg_tde encryption-at-rest through the Custom Resource, so regulated and sovereignty-minded teams meet database-level encryption requirements without installing the extension as a custom package or hand-wiring key providers. Teams enable encryption in the cluster spec, supply HashiCorp Vault connection details or a file-based key, and store credentials in a Secret; the Operator configures the key provider. [Percona Transparent Data Encryption for PostgreSQL](https://docs.percona.com/pg-tde/), [PostgreSQL Operator Custom Resource options](https://docs.percona.com/percona-operator-for-postgresql/latest/operator.html)
...
- **Multi-cloud and hybrid portability:** Percona Operators run on any CNCF-conformant Kubernetes, including OpenShift, Amazon EKS, Google GKE, Azure AKS, Rancher RKE2, and on-prem clusters. Official RKE2 validation lists that distribution as a supported platform for operators such as Percona Operator for PostgreSQL and Percona Operator for MongoDB, so teams standardized on Rancher can deploy without performing their own platform certification. Teams can move workloads freely without rewriting automation or getting trapped by a single cloud vendor.
  - **MongoDB:** Same Operator model on ARM servers. Multi-cluster and multi-region setups support teams that need data in more than one place.
  - **MySQL:** [Cross-site replication](https://docs.percona.com/percona-operator-for-mysql/pxc/dr-replication.html) keeps clusters in sync across Kubernetes sites.
