...
- Which operator owns PostgreSQL on Kubernetes today (CloudNativePG, Crunchy, Zalando, Percona), and are backups Barman Cloud object-store, pgBackRest, or something else? If the estate is CloudNativePG (CNPG), ask what commercial support must cover and who owns Day-2 work. If the estate is Crunchy, ask destination preference (Percona Operator for PostgreSQL vs CloudNativePG), sunset or support pressure, and whether Ansible automation must carry forward. (See [Support for PostgreSQL](https://www.percona.com/services/support/postgresql-support).)
