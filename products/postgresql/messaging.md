...
- Legacy RDBMS exit to PostgreSQL for TCO: Assessment-led database modernization and migration from proprietary and legacy commercial databases helps teams cut license spend on [Percona Distribution for PostgreSQL](https://www.percona.com/postgresql/software) through [Percona Expert Consulting and Services](https://www.percona.com/services/consulting). MySQL, MariaDB, Oracle, and SQL Server to PostgreSQL are common heterogeneous modernization paths; Percona delivers schema migration, code conversion, data migration and continuous replication, schema and data validation, and cutover.
...
- End-to-end encryption: Percona integrates pg_tde for database-level transparent data-at-rest and WAL encryption (not storage-only encryption alone), including 256-bit AES, optimized encrypted I/O, and external key management through HashiCorp Vault, Thales CipherTrust, Fortanix SDKMS, Open Bao, and Akeyless. On Kubernetes, Percona Operator for PostgreSQL enables pg_tde through the Custom Resource with Vault or a file-based key, so teams do not treat encryption as a custom-extension add-on. Encrypted-cluster upgrades are supported through pg_tde_upgrade and validated major-version upgrade paths on the distribution stack. Current constraints and KMS options are documented in [Percona Transparent Data Encryption for PostgreSQL](https://docs.percona.com/pg-tde/).
...
- Cloud-native operations: The Percona Operator for PostgreSQL automates deployment, scaling, and failover in Kubernetes environments, delivering consistent governance and portability across supported platforms, including Rancher RKE2.
