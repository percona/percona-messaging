...
- **AI-ready databases:** Vector and retrieval support differs by engine.
  ...
  - **MongoDB:** Percona Search for MongoDB is an optional add-on for Percona Server for MongoDB. It adds full-text and vector search on data you already keep, without a second search system. This Technical Preview needs Percona Server for MongoDB 8.3, is for staging not production, and you provide embeddings yourself. More Search capability is planned in upcoming releases. On Kubernetes, the Percona Operator for MongoDB can install and run Search with the cluster.
