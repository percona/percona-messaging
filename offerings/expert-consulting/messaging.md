...
- **Legacy exit and lifecycle transitions:** When license, vendor, or EOL pressure drives the case and teams are unsure of the right path, assessment scopes migration, modernization, EOL planning, or Extended Lifecycle Support (ELS). Percona then delivers what fits through schema migration, code conversion, data migration and continuous replication, schema and data validation, and cutover. Complex legacy estates often run through the **PACE** program; for complex heterogeneous moves, Percona partners with HexaCluster and you contract with Percona. Optional post-migration Expert Support follows cutover.
